...
public class ParseXMLUtil {
	...
	/** 开始解析xml文件 **/
	public ParseXMLUtil(InputStream xmlInputStream) {
		try {
	...
	public void setColumnMap(String entityName, Element column) {
		if (column != null) {
			...
			if (type.equals("String")) {
				String minLength = column.attributeValue("minLength");
				String maxLength = column.attributeValue("maxLength");
				col.put("minLength", minLength);
				col.put("maxLength", maxLength);
			} else if (type.equals("int") || type.equals("double")) {
				String min = column.attributeValue("min");
				String max = column.attributeValue("max");
				col.put("min", min);
				col.put("max", max);
			} else if (type.equals("Date")) {
				String format = column.attributeValue("format");
				col.put("format", format);
			}
